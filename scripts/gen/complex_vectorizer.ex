defmodule SciEx.Gen.ComplexVectorizer do
  alias SciEx.Gen.{
    RustComplexModule,
    Function,
    VectorizedArgument
  ...
  def vectorize_type(function, "Complex<f64>"), do: "ExComplex64Array#{function.n_dim}"
  def vectorize_type(function, "Complex<f32>"), do: "ExComplex32Array#{function.n_dim}"
  def vectorize_type(_function, other), do: other

  def elixirize_type(_function, "Complex<f64>"), do: "ExComplex64"
  def elixirize_type(_function, "Complex<f32>"), do: "ExComplex32"
  def elixirize_type(_function, other), do: other

  def maybe_vectorize_argument(function, arg, :scalar) do
    %VectorizedArgument{
      name: arg.name,
      type: elixirize_type(function, arg.type),
  ...
  def maybe_vectorize_argument(function, arg, :array) do
    case arg.type do
      ty when ty in ["f64", "f32", "Complex<f64>", "Complex<f32>"] ->
        %VectorizedArgument{
          name: arg.name,
  ...
  def vectorize_function(%RustComplexModule{} = rust_module, function) do
    functions =
      for n_dim <- 1..(rust_module.n_dims) do
        ...
        for choice <- vectorization_choices do
          ...
          if vectorization_possible? do
            ...
            prefix = "#{rust_module.prefix}_complex#{rust_module.bits}"
            ...
            if Enum.all?(choice, fn x -> x == :scalar end) do
              if n_dim == 1 do
                ...
                %{
                  function_with_dim |
                    returns_array: false,
                    result_type: elixirize_type(function_with_dim, function_with_dim.result_type)
                }
              else
  ...
  @external_resource "scripts/rust_generator/templates/vectorized_complex_function.rs"

  EEx.function_from_file(
    :def,
    :vectorized_function_rs,
    "scripts/rust_generator/templates/vectorized_complex_function.rs",
    [:assigns]
  )

  @external_resource "scripts/rust_generator/templates/vectorized_complex_function_one_arg.rs"

  EEx.function_from_file(
    :def,
    :vectorized_function_one_arg_rs,
    "scripts/rust_generator/templates/vectorized_complex_function_one_arg.rs",
    [:assigns]
  )

  @external_resource "scripts/rust_generator/templates/scalar_complex_function.rs"

  EEx.function_from_file(
    :def,
    :scalar_function_rs,
    "scripts/rust_generator/templates/scalar_complex_function.rs",
    [:assigns]
  )
  ...
  def vectorized_function_to_rust_code(function) do
    ...
    f_args =
      for arg <- function.arguments do
        cond do
          arg.type in ["ExComplex64", "ExComplex32"] ->
            "#{arg.name}.resource.0"

          String.contains?(arg.type, "ExComplex") and String.contains?(arg.type, "Array") ->
            arg.name
    ...
    if function.returns_array do
      vectorized_function_rs(
        ...
        f_args_as_refs_azip: f_args_as_refs_azip,
        f_args_as_refs_par_azip: f_args_as_refs_par_azip,
        f_args: f_args,
        array_arg: array_arg,
  ...
  def vectorized_functions_to_rust_code_iodata(rust_module, functions) do
    ...
    use ndarray::{azip, par_azip, #{array_types_list}};
