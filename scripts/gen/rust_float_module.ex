defmodule SciEx.Gen.RustFloatModule do
  ...
  alias SciEx.Gen.{RustParser, FloatVectorizer, Injector}

  require EEx

  @external_resource "scripts/rust_generator/templates/sci_ex_test_math_functions_test.rs"
  ...
  defp bulk_assign_type_to_self(functions, type) do
    Enum.map(functions, fn f -> FloatVectorizer.assign_type_to_self(f, type) end)
  end

  defp bulk_rename_self(functions, new_name) do
    Enum.map(functions, fn f -> FloatVectorizer.rename_self(f, new_name) end)
  end
  ...
  def add_vectorized_functions_from_file(rust_module, path, opts \\ []) do
    ...
    vectorized_functions = FloatVectorizer.vectorize_functions(rust_module, functions)

    %{rust_module | functions: rust_module.functions ++ vectorized_functions}
  end

  def to_rust_code_iodata(rust_module) do
    FloatVectorizer.vectorized_functions_to_rust_code_iodata(
      rust_module,
